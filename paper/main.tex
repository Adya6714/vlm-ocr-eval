...
\title{Reading Without Looking: A Position-Resolved Profile of\\Confidence and Correctness in a Small Document OCR Model}
...
independently trained seeds. We first establish what the model can do: on held-out Hindi
images its grapheme character error rate is 0.985, and on blank white images it is 0.949. The
model does not read the held-out images, which are drawn from GlotOCR Bench and rendered by an
independent pipeline, and text-bearing and blank input are not distinguishable at three seeds
($t = 0.257$, $p = 0.822$, 2 df). We then localise the
...
correct first symbol is $2.2\times10^{-11}$ on text-bearing images and $6.3\times10^{-11}$ on blank:
more than eight orders of magnitude below what a text-only $n$-gram model fitted on the same
training corpus, which never sees an image, assigns to that same symbol. This is confirmed by a
scale-free statistic immune to softmax-sharpness objections: the median rank of the correct
symbol among 367 candidates is 70.5, with 35.0\% of sequences ranking it below position 100.
Self-generated max-softmax confidence at that position is nonetheless approximately 0.90. From position~2
...
synthetic lines drawn entirely from the training manifest, where the model attains 18.3\% line
accuracy, confidence discriminates correct from incorrect transcriptions at AUROC 0.838 while
remaining badly miscalibrated at a mean of 0.994 --- a discrimination result we can state only
for memorised text, since this evaluation set has no held-out arm; on the held-out images, where
accuracy is zero, it carries no usable correctness signal. We read this as a model whose certainty tracks the linguistic plausibility
...
for transcription should weight early positions rather than averaging uniformly. We do not
exclude the possibility that this instrument never acquired a visual representation that
transfers to the evaluation renderer, and we state what would separate that from visual bypass.
\end{abstract}
...
(1)~We establish by direct measurement that the instrument does not read its held-out images
(grapheme CER 0.985 text-bearing against 0.949 blank, a difference that does not survive seed
...
contrast rather than a purely negative result; we are explicit that this evaluation set has no
held-out arm, so the result is stated for memorised text and not offered as evidence of
generalisation.
(5)~We show that \emph{at least} 16.9--20.4\% of non-exact predictions from off-the-shelf OCR
engines on Indic document images are Unicode encoding variants rather than misreadings, on the two
...
token corresponds to one decision about what was read. Vocabulary size is $|V| \approx 367$, built from
the run's own manifest with a minimum grapheme frequency of 5. Three seeds were trained
...
\paragraph{Where the images come from.} Both training and evaluation text originate in the
Hindi portion of GlotOCR Bench \citep{kargaran2026glotocr}, but the images do not. Training
images are produced by our own renderer from GlotOCR Hindi ground-truth strings. Evaluation
images are GlotOCR Bench's own renders of Hindi sentences, in its two variants: a clean
rendering on a white canvas and a degraded rendering that applies paper texture, ink wear,
geometric distortion, downsampling and compression artifacts. \textbf{They are renders, not
photographs or scans of physical documents}, and we do not describe them as scans anywhere in
this paper. The distribution shift the instrument faces at evaluation is therefore between two
rendering pipelines rather than between synthetic and photographed text, which is a smaller gap
than a scan corpus would present. Throughout, we write \emph{text-bearing} for evaluation images containing rendered text, in
contrast to \emph{blank} white images. We avoid the word ``real'' for our own data entirely,
since none of it is photographed.
...
We apply this taxonomy to three off-the-shelf OCR engines on the same image corpus. The scope
...
\section{The Instrument Does Not Read Its Held-Out Images}
\label{sec:cer}
...
\midrule
Hindi, held-out ($n=60$/seed)    & 1.009 & 0.984 & 0.963 & \textbf{0.985} \\
...
First, \textbf{the model does not read its held-out images}. A CER near 1.0 is not partial
...
$p = 0.197$. Note that blank outputs are not shorter than text-bearing ones (mean grapheme length
...
strings. \textbf{The defensible claim is that text-bearing and blank CER are statistically
...
fails on held-out images of its own training script as thoroughly as on scripts it has never seen.

The consequence for framing is direct. We cannot claim a dissociation between reading ability
and confidence, because on these images there is no reading ability to dissociate from. What we
...
\paragraph{Bypass or domain shift?} A reader may object that this instrument was trained on
images from our own renderer and evaluated on images from GlotOCR Bench's, so it may have no
usable visual representation for the evaluation distribution at all. That is domain shift
rather than bypass, and the two are different failures. We note that the gap is narrower than
the objection usually assumes: both sides are rendered text from the same source corpus, in the
same script, at the same canonical line height, differing in rendering stack, font and
degradation pipeline rather than in medium. Our own evidence cuts
...
position profile is near-identical on text-bearing and blank input at every bucket
...
domain shift alone predicts. We also ran the two measurements that would separate them:
Gaussian-noise images and patch-scrambled real images, testing whether the model responds to
visual variation of any kind rather than only to the specific rendering gap between training
and evaluation. Pooled whole-sequence mean $\log p(\mathrm{GT})$ is $-1.756$ on noise and
$-1.730$ on patch-scrambled input, against $-1.783$ text-bearing and $-1.751$ blank --- all four
conditions within 0.05 nats of one another, and the position-bucket profile for both new
conditions reproduces the same shape as text-bearing and blank: collapse at positions 0
($-23.7$ noise, $-24.1$ scrambled) and 1 ($-8.9$, $-7.9$), recovery through positions 2--39, and
a second collapse beyond position 40. Pure noise carries no structure a domain-shift account
would expect the model to process differently from a familiar-but-shifted rendering, and
patch-scrambled input destroys spatial layout while preserving local texture statistics; if the
model retained any domain-specific visual competence, these two conditions should diverge from
text-bearing input more than blank does. They do not. This is evidence for bypass over domain
shift, though we note it is evidence, not proof: an instrument that never learned to use images
under any condition is consistent with both a total bypass failure and a domain-shift failure
severe enough to leave no residual sensitivity to any visual input at all.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig6_noise_scrambled.pdf}
\caption{Text-bearing, blank, Gaussian-noise, and patch-scrambled input compared. \textbf{Left:}
whole-sequence mean $\log p(\mathrm{GT})$, per-seed points with the pooled mean (dashed).
\textbf{Right:} $\log p(\mathrm{GT})$ at position 0 specifically, same layout. All four
conditions cluster tightly on both measures; noise and patch-scrambled input are
indistinguishable from text-bearing and blank rather than falling between them, which is the
pattern a residual, domain-specific visual competence would predict and does not appear here.}
\label{fig:noisescrambled}
\end{figure}

We also state the training budget plainly, because it bears on this and a reader will reach it
in Appendix~\ref{app:config} otherwise: 2{,}538 rendered line crops, a single unrecorded font, no
...
model may simply never have acquired a visual representation that transfers across renderers.
We
...
Condition & Seed 0 & Seed 1 & Seed 2 & Pooled \\
\midrule
Hindi, text-bearing ($n{=}60$) & 0.9824 $\pm$ 0.0198 & 0.9897 $\pm$ 0.0085 & 0.9899 $\pm$ 0.0119 & 0.9873 \\
...
more variation in reported confidence than whether the input carries text, an unfamiliar
...
all 60, one accounting for 86.7\%. On text-bearing Hindi the corresponding counts are 10, 9 and 10
...
mechanism than fluent varied hallucination and we do not describe it as the latter. Text-bearing input
is also far from diverse: 22 to 30 unique outputs across 60 distinct images.

\subsection{Mismatched pairing rules out the degeneracy confound}
\label{sec:mismatch}

Blank is a degenerate input by construction, so ``blank scores similarly to text-bearing'' is
weaker evidence than a design that removes degeneracy entirely: score each ground-truth string
against a \emph{different, real} image, under a fixed derangement of the 60-image pool, so
every comparison image is a genuine text-bearing photograph and none is a repeat of its own
match. Mean teacher-forced $\log p(\mathrm{GT})$ under this mismatch is $-1.552$ (seed~0),
$-1.702$ (seed~1), $-1.871$ (seed~2); medians $-1.133$, $-1.326$, $-1.436$. These sit inside the
same range as the correctly-paired text-bearing condition reported in
Section~\ref{sec:position} ($-1.783$) and the blank condition ($-1.751$), not below them. The
position-bucket breakdown is the same shape too: pooled mean $\log p(\mathrm{GT})$ under
mismatch is $-23.80$ at position~0, $-7.82$ at position~1, recovering to $-0.47$, $-0.18$ and
$-0.24$ across positions 2--39, and collapsing again to $-5.90$ beyond position~40 --
indistinguishable in shape from Table~\ref{tab:position}'s text-bearing and blank rows. Scoring
a caption against a real photograph of an entirely different page produces the same
likelihood profile as scoring it against its own correct image or against a blank one. The
degeneracy explanation in Section~\ref{sec:degeneracy} accounted for why blank output looks
artificially close to text-bearing output; it cannot account for this, since the comparison
images here are genuine, varied photographs, not a collapsed handful of repeated strings. This
closes the alternative that the paper's central result is an artefact of what the decoder does
on blank pages specifically.
...
text-bearing Hindi is 9\% image / 7\% seed / 84\% residual, and on blank 0\% / 18\% / 82\%.
Ground-truth likelihood behaves oppositely at 84--85\% image / $\approx$2\% seed /
$\approx$14\% residual. Which image is presented explains 9\% of confidence variance on text-bearing
...
give, for confidence on text-bearing Hindi, image 9.1\% [0.0, 22.4], seed 7.1\% [0.2, 17.7] and
...
[69.7, 91.1] on text-bearing and 85.0\% [70.8, 90.9] on blank.
...
image$\times$seed interaction. And the image share for confidence on text-bearing input has an interval
...
text-bearing images. Divergences are reported as $\mathrm{KL}(p_{\text{full}} \,\|\, p_{\text{zero}})$
throughout.
...
\paragraph{Cross-attention contribution norms.} The behavioural claim above, that the encoder
contributes something but not much, has a direct architectural measurement:
$\|\text{cross-attn output}\|/\|\text{residual stream}\|$ per decoder layer, averaged over
positions, computed separately on text-bearing and blank input across all three seeds and all
five decoder layers. The pattern is consistent across every seed: the cross-attention pathway's
contribution to the residual stream is small in absolute terms (0.10--0.13 across layers and
conditions) and is \emph{higher on text-bearing input than on blank at every layer, in every
seed} (e.g.\ seed~0, layer~0: 0.1131 blank versus 0.1202 text-bearing; seed~1, layer~2: 0.1183
versus 0.1427). The gap is modest, ranging roughly 0.01--0.02 in absolute norm depending on
layer and seed, but its direction is uniform across all 15 (seed, layer) cells. This is
independent, architecture-level evidence that the encoder pathway is not inert: it carries
measurably more signal when there is something in the image to encode, even though that
additional signal does not translate into correct output at position~0 or into a lower
self-reported confidence when it is absent. Full per-layer, per-seed values are in
Appendix~\ref{app:crossattn}.
...
(SD 1.274, $n=180$) on text-bearing input and $-1.751$ (SD 1.280, $n=180$) on blank, with mean
...
text-bearing and blank, and the nominal signs should not be interpreted. The two estimators are also
...
of positions on text-bearing input and 90.1\% on blank, with a mean peak probability of 0.9983 and
0.9982 at those positions, consistent with the reported entropy. Broken out by position:
\textbf{0\% at position 0}, on 0 of 180 sequences, under both conditions; 92.6\% (text-bearing) versus
...
model references fitted on 2{,}491 of the 2{,}538 training lines (rows matching an evaluation
string removed, add-$\alpha$ smoothing with $\alpha = 0.01$; note that the instrument itself was
\emph{not} held out this way, see Appendix~\ref{app:config}) and evaluated \emph{within the same position
...
Positions & $n$ tok. & Text-bearing & Blank & 1-gram & 2-gram & 3-gram & 4-gram & 5-gram \\
\midrule
...
position, text-bearing and blank overlaid. The horizontal reference lines drawn here are
...
between text-bearing and blank input.}
\label{fig:position}
\end{figure}
...
probability assigned to the correct first symbol is $2.2\times10^{-11}$ on text-bearing images and
...
One further fact makes this harder to explain away rather than easier. The instrument was
trained on the full manifest with no held-out split, and 19 of the 60 evaluation strings appear
verbatim as training lines (Appendix~\ref{app:config}), while the $n$-gram references were
fitted with the exactly-matching rows removed. The asymmetry is real but modest, and we state it
at its true size: the reference is deduplicated rather than held out, since the other 41
evaluation strings are resampled from the same pool and their contexts remain in the fit, and
dropping 47 of 2{,}538 rows moves a position-0 marginal by under two percent. What survives is
the direction: the comparison at position~0 favours the instrument, and a model that has seen a
third of these strings verbatim during training still never places the correct first symbol at
the argmax and still assigns it roughly 20 nats less probability than the reference.

Two scale-free quantities sharpen the claim further, and both are now measured. Over the same
180 (image, reference) pairs, the median rank of the ground-truth symbol among the 367
candidates is 70.5 pooled (seed medians 60.5, 59.5, 95.0), IQR [33.0, 136.2], with 35.0\% of
sequences ranking the correct symbol below position 100 in the model's own distribution. This
is a scale-free statistic, immune to the softmax-sharpness objection raised above: a model that
places the truth at a median rank of 70 out of 367 is not merely confident about the wrong
symbol, it is actively disfavouring the correct one relative to hundreds of alternatives. Mean
predictive entropy at position~0 is 0.333 nats pooled, low enough to confirm the distribution is
sharply peaked, consistent with the near-ceiling confidence reported next. The probability
assigned to an arbitrary non-argmax symbol is also measured (Appendix~\ref{app:pos0}) and
remains orders of magnitude above $p(\mathrm{GT})$, confirming the collapse is specific to the
correct symbol rather than an artefact of general softmax sharpness. The memorisation question
for Section~\ref{sec:works} is resolved, not outstanding: the synthetic evaluation set has no
held-out arm, so the split cannot be run, and Section~\ref{sec:works}'s AUROC is reported as an
in-training-manifest figure accordingly.

\paragraph{2. Confidence at that position is near ceiling.} Self-generated max-softmax at
position 0 is 0.902 pooled on text-bearing input and 0.895 on blank. Position 0 is therefore the point
of maximal confidence and minimal correctness, at the one generation step where vision is the
only possible information source. Per seed, text-bearing: 0.884 / 0.906 / 0.916; blank: 0.9999 / 0.898
...
second generated symbol sits 7.0 nats (text-bearing) and 7.5 nats (blank) below the best
...
without reference to the image, since the blank column matches the text-bearing column at every bucket.

Two properties of this comparison make the mid-sequence agreement weaker evidence than it
looks, and we would rather state them than have them found. First, the two sides are not held
out symmetrically: the $n$-gram references drop every manifest row matching an evaluation
string, while the instrument was trained on the full manifest, so at positions where the
instrument matches or exceeds the 5-gram reference it is doing so with an advantage the
reference does not have. The $-0.15$ at positions 10--19, the one bucket where the instrument
beats the 5-gram model, is exactly where that advantage would show. We therefore do not claim
the instrument outperforms a high-order grapheme prior; we claim only that its mid-sequence
likelihood sits in that band under a comparison it wins by construction.

Second, the training text is drawn from the same
...
matching a generic one, but the comparison should be read with that in mind. It also means the
18.3\% synthetic line accuracy in Section~\ref{sec:works} likely \emph{overstates} genuine
reading ability, since the evaluation text is drawn from the same resampled pool as the training
text; we report it as a regime marker rather than as a capability estimate.

A distributional comparison, replicated across all three seeds, supports the same picture.
Per-position $\mathrm{KL}$ between the model and the 5-gram reference, together with the rate
at which their argmaxes coincide, pooled over $n=180$: at positions 2--9, 10--19 and 20--39,
mean KL is 0.32, 0.27 and 0.28 nats and argmax agreement is 93.5\%, 91.2\% and 90.7\% --- the
model and the 5-gram reference are not just close in mean log-likelihood, they select the same
token at the overwhelming majority of mid-sequence positions, consistently across every seed
(per-seed agreement at positions 2--9 ranges 92.9--93.8\%). At positions 0, 1 and 40+, where
the earlier observations report the instrument falling far below the $n$-gram baselines, KL
rises sharply (4.98, 1.28 and 1.92 nats pooled) and argmax agreement collapses (4.4\%, 62.8\%,
61.5\%). The two measures move together exactly where the position profile changes shape,
replicated at every seed, which is what a genuine 4-to-5-gram account predicts and a
coincidental mean-matching would not. We regard the exclusive claim as supported.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig7_ngram_kl.pdf}
\caption{Per-position-bucket $\mathrm{KL}(\text{model} \,\|\, \text{5-gram})$ (left) and argmax
agreement rate between the model and the 5-gram reference (right), pooled bars with per-seed
points overlaid. Both measures move together: low KL and high agreement in the
recovered mid-sequence region, both collapsing sharply at positions 0, 1, and 40+ --- exactly
where the position profile itself changes shape.}
\label{fig:ngramkl}
\end{figure}
...
Held-out Hindi images              & 180 & 0.000 & undefined & $-0.15$ \\
Held-out Hindi, median CER split   & 180 & ---   & 0.57 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
curve is against line-level exact correctness; the held-out curve is against a median split on
grapheme CER, because exact accuracy there is identically zero and the AUROC against it
...
Before the numbers, a scoping note that governs all of them, and it is stronger than we
initially stated. The instrument was trained on the full rendered manifest with no held-out
split (Appendix~\ref{app:config}), and we checked directly whether the synthetic evaluation
lines behind this section's numbers are held out from that manifest: they are not, for any of
them. All 300 evaluation instances (100 ground-truth strings $\times$ 3 seeds) are sampled from
the training manifest itself, so there is no non-matching subset to compare against --- the
memorisation-versus-correctness split we describe below was attempted and could not be run,
because the design has no held-out arm. The 18.3\% line accuracy and the AUROC that follows are
therefore \textbf{in-training-manifest figures only}. They do not show, and cannot show with
this evaluation set, that confidence tracks correctness on text the model has not seen. We use
them here only as a \emph{regime marker}: this identifies a setting in which the model produces
some correct output on training-distribution text, so that the confidence signal's behaviour in
that setting can be compared against the setting where it produces none, not as evidence about
generalisation.

The alternative we cannot exclude is therefore not merely plausible but the default reading: if
the lines the model transcribes exactly are disproportionately ones it memorised, a confidence
signal that ranks them highly may be tracking memorisation rather than correctness, and the two
hypotheses produce the same AUROC on this evaluation set by construction. Testing which is true
requires a held-out evaluation split that does not currently exist for the synthetic condition,
and building one is future work, not a rerun of an existing probe.

On these synthetic lines, where the model gets 18.3\% of lines exactly right, confidence ranks
correct transcriptions above incorrect ones at AUROC 0.838.
...
come apart cleanly here, and only the former survives. On the held-out images, where accuracy is
...
cannot distinguish a working instrument from a broken one on this evidence alone. We attempted
Surya, one of the engines in Table~\ref{tab:tier1} and an architecture in the same encoder--decoder
family as our instrument, and it is not usable for this purpose: the deployed inference package
serves a llama.cpp-backed VLM with no exposed softmax and no separable encoder tensor to zero,
and the older checkpoint matching our intended architecture is incompatible with current loading
code (mismatched attention head dimensions, additional mixture-of-experts parameters with no
principled way to drop them). We declined to force the load with a mismatched-size override,
since that would silently produce meaningless weights rather than a real result. We attempted a
second candidate, PaddleOCR's own recognition backbone (already in Table~\ref{tab:tier1} and
confirmed to read Devanagari), and it fails for a more fundamental reason than Surya did: the
installed recognition model (\texttt{devanagari\_PP-OCRv5\_mobile\_rec}) is CTC-based, not
autoregressive. Its SVTR component is a visual encoder over the feature map, not a text decoder;
output is a softmax over the charset at each spatial column, collapsed by CTC decoding, with no
beginning-of-sequence token and no $p(g_i \mid g_{<i}, \text{image})$ to teacher-force. There is
no generation loop to extract per-step logits from, and no natural mapping from CTC's column-wise
output to our grapheme-cluster vocabulary. Reading column-0 softmax as a substitute for
position-0 teacher-forced likelihood would not measure the same quantity, so we do not report
one. Broader benchmarks of real-scan Devanagari competence \citep{singh2026devanagari} identify
stronger candidates, though the top-scoring systems there are closed and do not expose per-step
likelihoods. A viable positive control remains the single most valuable open experiment, and
both candidates attempted so far have failed for architectural reasons rather than for lack of
trying: one serves an inference stack with no exposed distribution at all, the other has no
autoregressive distribution to expose in the first place.

\paragraph{What remains.} Mismatched pairing (Section~\ref{sec:mismatch}), cross-attention
contribution norms (Section~\ref{sec:ablation}), noise and patch-scrambled input
(Section~\ref{sec:cer}), and the $n$-gram KL/argmax-agreement decomposition
(Section~\ref{sec:position}), replicated across all three seeds, are now measured and no
longer listed here. The memorisation question for Section~\ref{sec:works} is also resolved,
though not in the way we originally planned: we checked whether the synthetic evaluation set
has a non-matching subset to split against, and it does not, so the split cannot be run on this
evaluation set. What remains is therefore not a re-analysis but a new evaluation set: synthetic
lines rendered from text that provably does not appear in the training manifest, generated and
held out before any training occurs, would let Section~\ref{sec:works}'s AUROC be tested for
generalisation rather than reported as an in-training-manifest figure. This requires new
rendered data, not new model training or new GPU time. A positive control on a model with
demonstrated reading ability remains the single item in this list that could not be completed
within available compute; the probe implementations needed for it are written and released with
the code, so running them is a matter of finding a compatible model, not further development.

\paragraph{No held-out training split.} The instrument was trained on the entire rendered
manifest, with no evaluation strings withheld; 19 of the 60 strings used for the teacher-forced
analyses appear verbatim as training lines, and the remainder are resampled from the same source
pool. This is a design fault rather than an analysis choice, and a redesign should hold out the
evaluation strings before training rather than after. Its consequences are not uniform across
the paper and we have tried to mark each one where it falls. It inflates the synthetic line accuracy in
Section~\ref{sec:works}, which we therefore use only as a regime marker, and it removes the
possibility of testing whether the AUROC reported there measures confidence tracking correctness
or confidence tracking memorisation: we checked directly, and all 300 evaluation instances are
drawn from the training manifest with no non-matching subset to compare against, so the split we
originally proposed cannot be run on this evaluation set. Section~\ref{sec:works}'s AUROC should
be read as an in-training-manifest figure, not as evidence about generalisation. It biases the mid-sequence comparison in Section~\ref{sec:position} in the instrument's
favour, since the $n$-gram references were held out and the instrument was not, so we do not
claim the instrument beats a high-order prior. It does not weaken the position-0 and position-1
results, which are the paper's central measurements: contamination can only have helped the
instrument there, and it still fails at floor. And it is orthogonal to the encoder-ablation
results in Section~\ref{sec:ablation}, which compare the same model against itself on the same
images.
...
read its held-out images. We localised the failure to the start of the generated sequence,
...
$1.35\times10^{-11}$ / $3.88\times10^{-12}$ (text-bearing) and $9.32\times10^{-10}$ /
...
(text-bearing) and $3.7\times10^{-13}$ (blank); the arithmetic means are $2.67\times10^{-3}$ and
$1.51\times10^{-3}$, close to uniform, but they are set by a handful of sequences. Only
2.8\% of text-bearing sequences (5 of 180) and 3.3\% of blank sequences (6 of 180) assign the correct
...
\begin{tabular}{lrrrr}
\toprule
Bin ($\log p$) & Text-bear. count & \% & Blank count & \% \\
\midrule
...
Seventy percent of text-bearing sequences place $\log p(\mathrm{GT}) < -25$ at position 0. The
collapse is a property of the distribution, not of its summary.

\paragraph{Rank of the ground truth (E1).} Table~\ref{tab:ranke1} gives the full per-seed and
pooled rank statistics referenced in Section~\ref{sec:position}. Rank is computed over the
367-symbol vocabulary at position 0 under teacher forcing, on text-bearing input, $n=60$ per
seed.

\begin{table}[h]
\small
\centering
\caption{Rank of the ground-truth symbol at position 0, text-bearing input. $\bar{H}$ is mean
predictive entropy in nats.}
\label{tab:ranke1}
\begin{tabular}{lrrrr}
\toprule
 & Median rank & IQR & $P(\text{rank}>100)$ & $\bar{H}$ \\
\midrule
Seed 0   & 60.5 & [28.2, 104.5] & 0.267 & 0.492 \\
Seed 1   & 59.5 & [28.2, 129.2] & 0.333 & 0.285 \\
Seed 2   & 95.0 & [54.5, 176.5] & 0.450 & 0.222 \\
Pooled   & 70.5 & [33.0, 136.2] & 0.350 & 0.333 \\
\bottomrule
\end{tabular}
\end{table}

\section{Cross-Attention Contribution Norms}
\label{app:crossattn}

Full per-layer, per-seed values for $\|\text{cross-attn output}\|/\|\text{residual stream}\|$
referenced in Section~\ref{sec:ablation}, computed over text-bearing and blank input, $n=60$
images per seed.

\begin{table}[h]
\small
\centering
\caption{Cross-attention contribution norm by decoder layer (L0--L4), seed, and condition.}
\label{tab:crossattn}
\begin{tabular}{lrrrrr}
\toprule
Seed, condition & L0 & L1 & L2 & L3 & L4 \\
\midrule
0, blank        & 0.1131 & 0.1232 & 0.1247 & 0.1188 & 0.1068 \\
0, text-bearing & 0.1202 & 0.1306 & 0.1273 & 0.1191 & 0.1095 \\
1, blank        & 0.1030 & 0.1172 & 0.1183 & 0.1142 & 0.1026 \\
1, text-bearing & 0.1269 & 0.1427 & 0.1427 & 0.1347 & 0.1249 \\
2, blank        & 0.1025 & 0.1141 & 0.1217 & 0.1100 & 0.1047 \\
2, text-bearing & 0.1214 & 0.1316 & 0.1337 & 0.1234 & 0.1156 \\
\bottomrule
\end{tabular}
\end{table}

Text-bearing exceeds blank at every one of the 15 (seed, layer) cells, though the margin is
modest (0.007--0.020 in absolute norm) relative to the total contribution (0.10--0.14).
...
unpaired tests. Per-seed Wilcoxon signed-rank on grapheme CER (text-bearing minus blank) gives
...
grapheme CER 0.9855 [0.9406, 1.0347] text-bearing and 0.9491 [0.9053, 0.9911] blank; mean confidence
0.9873 [0.9852, 0.9892] text-bearing, 0.9857 [0.9838, 0.9874] blank, 0.9857 [0.9843, 0.9871] Ol Chiki
and 0.9894 [0.9871, 0.9916] Perso-Arabic; position-0 geometric mean $p(\mathrm{GT})$
$2.20\times10^{-11}$ [$1.04\times10^{-11}$, $5.29\times10^{-11}$] text-bearing; AUROC 0.838
[0.771, 0.902] on synthetic natural and 0.570 [0.491, 0.650] on the held-out images.
...
\paragraph{Evaluation data.} Evaluation images are GlotOCR Bench renders, not scans, in that
benchmark's two released rendering profiles, \textsc{plain} and \textsc{old\_document}; the
latter simulates an aged document through paper texture, ink wear, geometric distortion,
downsampling and compression artifacts.
They are resized to height 70 preserving aspect
ratio (Lanczos). The
model-side evaluation uses 60 Hindi rows in both rendering variants, 100 Ol Chiki rows
...
\paragraph{Train/evaluation overlap.} \texttt{train.py} does not hold out the 60 evaluation
strings. \texttt{LineDataset} appends every JSONL row of the supplied manifest, the vocabulary
is built from every \texttt{text} field of that same file, and the documented Hindi run passes
\texttt{data/manifests/hindi\_natural.jsonl} (2{,}538 lines) with no filter. The $n$-gram
references in Section~\ref{sec:position} are the only fit that drops evaluation-matching lines:
a manifest row is kept unless its \texttt{text} equals one of the 60 evaluation ground-truth
strings, which leaves 2{,}491 lines. That is not $2{,}538-60=2{,}478$. Only 19 of the 60
evaluation strings occur as a manifest line, and those 19 occupy 47 rows because some strings
repeat; $2{,}538-47=2{,}491$. The other 41 evaluation strings have no exact line match, so they
are not among the dropped rows.

\section{Sequence Lengths}
\label{app:lengths}

Reference transcriptions average roughly 40 graphemes (2{,}397 tokens over 60 strings, from
Table~\ref{tab:position}), so the instrument under-generates by about 30\% in both conditions.
Mean and median generated grapheme lengths are 27.96 / 28.0 (text-bearing Hindi), 31.39 / 29.0
(blank), 28.28 / 29.0 (Ol Chiki) and 33.40 / 35.0 (Perso-Arabic), $n$ as in
Table~\ref{tab:cer}. Blank generations are not shorter than text-bearing ones, which rules out
