...
miscalibrated at a mean of 0.994; on the held-out images, where accuracy is zero, it carries no usable
...
correct symbol rather than an artefact of general softmax sharpness. The memorisation split for
Section~\ref{sec:works} remains outstanding and is listed in Section~\ref{sec:lim}.
...
Before the numbers, a scoping note that governs all of them. The instrument was trained on the
full rendered manifest with no held-out split (Appendix~\ref{app:config}), so the synthetic
evaluation lines in this section are drawn from the same rendered pool the model was trained on
and are not a held-out generalisation set. The 18.3\% line accuracy is therefore an
in-training-distribution figure, plausibly an in-training-set one for part of the sample, and it
overstates whatever reading ability the model would show on unseen text. We use it here only as
a \emph{regime marker}: it identifies a setting in which the model produces some correct output,
so that the behaviour of the confidence signal in that setting can be compared against the
setting where it produces none.

We flag a specific alternative that this design cannot exclude, because it concerns what the
AUROC measures rather than how large it is. If the lines the model transcribes exactly are
disproportionately the ones whose text it saw during training, then a confidence signal that
ranks them highly is tracking memorisation rather than correctness, and the two hypotheses
produce the same AUROC. Contamination raising accuracy makes that alternative more likely, not
less. Separating them requires splitting the synthetic evaluation lines by whether their text
has an exact manifest match and reporting accuracy and AUROC on each subset; we have not run
that split, and until it is run the 0.838 should be read as confidence tracking \emph{something}
that distinguishes the model's successes from its failures, without a commitment to which.
...
since that would silently produce meaningless weights rather than a real result. Broader
benchmarks of real-scan Devanagari competence \citep{singh2026devanagari} identify stronger
candidates, though the top-scoring systems there are closed and do not expose per-step
likelihoods. A viable positive control remains the single most valuable open experiment;
PaddleOCR's own recognition backbone, already in Table~\ref{tab:tier1} and confirmed to read
Devanagari, is a more promising candidate than Surya for a future attempt.

\paragraph{Two further interventions we consider necessary.} Mismatched pairing
(Section~\ref{sec:mismatch}) and cross-attention contribution norms
(Section~\ref{sec:ablation}) are now measured and no longer listed here. First,
\emph{Gaussian-noise and patch-scrambled inputs}, testing
whether the model responds to visual variation of any kind rather than only to the blank
versus text-bearing contrast; these are also what would separate visual bypass from the domain-shift
explanation raised in Section~\ref{sec:cer}. This probe is implemented; a dimension bug in the
noise-generation code (RGB source images were not converted to grayscale before generating
matched noise, producing a 3-channel array where the model expects one) was found and fixed but
the corrected version has not yet been re-run. A second would make Section~\ref{sec:position}'s
...
retaining the full per-step softmax, which the committed probe outputs do not. A fifth is
cheaper than any of these and settles a question raised in Section~\ref{sec:works}: split the
synthetic evaluation lines by whether their text appears verbatim in the training manifest, and
report line accuracy and AUROC on each subset. If the AUROC holds on the non-matching subset,
the confidence signal is tracking correctness; if it collapses, it is tracking memorisation.
This needs no model at all, only the committed predictions and the manifest. All four, together with the positive control above, are inference-only measurements requiring
...
Section~\ref{sec:works}, which we therefore use only as a regime marker, and it leaves open
whether the AUROC reported there measures confidence tracking correctness or confidence tracking
memorisation; splitting those evaluation lines by exact manifest match would settle it and is
listed among the measurements below. It biases the mid-sequence comparison in Section~\ref{sec:position} in the instrument's
