...
Two scale-free quantities would sharpen the claim further and neither is computed here: the
...
alongside the memorisation split for Section~\ref{sec:works}.
...
Table~\ref{tab:tier1}, attains 46.8\% exact match on Indic images including Devanagari, is
...
ground-truth string is scored against a different text-bearing image. A blank page is a degenerate
input, so ``blank equals text-bearing'' is weaker evidence than ``wrong image equals right
