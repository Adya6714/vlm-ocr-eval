...
We train a from-scratch encoder--decoder OCR model (the \emph{instrument}) so that
``this checkpoint has never seen this script'' is an architectural fact rather than an
assumption about a pretrained backbone. Architecture and optimiser are frozen across every
run; they are the values in \texttt{src/models/instrument/} rather than a search.

\paragraph{Architecture.} A ViT encoder (6 layers, width 320, 5 heads, MLP ratio 4, pre-norm)
embeds non-overlapping $14\times 14$ patches of a single-channel line crop of canonical height
70\,px, with sinusoidal patch positions. A linear map ($320\to 384$) bridges encoder memory
into an autoregressive decoder (5 layers, width 384, 6 heads, MLP ratio 4, pre-norm, learned
positions, maximum 128 tokens). The decoder output head is weight-tied to the token embedding.
The output vocabulary is grapheme clusters with a frequency floor of 5, not BPE: one token is
one visual decision. On the Hindi natural tokenizer this gives $|V|\approx 367$, so the
uniform reference is $1/|V|\approx 2.725\times 10^{-3}$. Instantiating
\texttt{InstrumentModel} at that vocabulary size yields $19{,}607{,}104$ trainable parameters
($\approx$19.5M). Training uses AdamW at $3\times 10^{-4}$, batch size 32, 5{,}000 steps, and
\texttt{fp16} on a Turing T4 (no \texttt{bf16}).

\paragraph{Nine checkpoints, not one.} Probe~1 trains this architecture independently on three
glyph-frequency corpora (natural / flattened / inverted) $\times$ three seeds: nine Hindi
checkpoints with isolated paths
\texttt{checkpoint\_hindi\_\{condition\}\_seed\{0,1,2\}.pt} (Decision \#14, \#47). Flattened
and inverted are not adapters on a shared natural run. Every mechanistic claim in
Sections~\ref{sec:cer}--\ref{sec:works} (real/blank CER, teacher-forced position profile,
encoder ablation, zero-shot scripts, calibration AUROC) uses only the three
\texttt{hindi/natural} seeds. Flattened and inverted enter solely as the failed exposure
experiment below: they show that changing the training PMF can collapse line accuracy, not
that visual bypass was re-measured on those weights.

\paragraph{Data.} Each condition is rendered as 100 pages of line crops
(\texttt{export\_manifest\_scaled.py --pages-per-mode 100}): 2{,}538 natural, 2{,}707
flattened, and 2{,}872 inverted Hindi lines. The renderer is specified to match a target
grapheme PMF within total-variation distance $0.08$ (\texttt{TARGET\_TV\_TOLERANCE}). Natural
lines match their empirical PMF by construction. Measuring the shipped flattened and inverted
manifests against the natural-support targets gives TV $0.22$ and $0.54$ respectively, so we
do not claim the 0.08 gate was met for those two conditions on the training files. They remain
an attempted frequency dial that also changed learnability.

\paragraph{What the exposure dial did.} Line accuracy, seed means over three seeds, is
$18.3\%\pm 5.1$ (natural), $0.33\%\pm 0.6$ (flattened) and $0.67\%\pm 0.6$ (inverted). With
the dependent variable at the floor in two of three conditions, a fixed-effects decomposition
of exposure against intrinsic script complexity is uninterpretable, and we withhold it
(Section~\ref{sec:lim}). Three natural seeds were required after a single-seed result failed
to replicate (Appendix~\ref{app:retraction}).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig1_position_dissociation.pdf}
\caption{The dissociation. \textbf{Top:} mean $\log p(\mathrm{GT})$ against generation
position, real and blank overlaid, with horizontal references at uniform ($-5.91$) and the
trigram grapheme language model ($-0.99$). \textbf{Bottom:} self-generated max-softmax
confidence at the same positions. Correctness collapses at positions 0--1 while confidence
does not, and both curves are near-identical between real and blank input.}
\label{fig:position}
\end{figure}
...
\paragraph{Scale and generality.} One architecture ($\approx$19.5M parameters; Section~\ref{sec:instrument})
and three independently trained \texttt{hindi/natural} seeds for the mechanistic experiments.
We make no claim about production OCR systems. A
